# A
```
scratches(划伤，损坏) and consoles  临时编辑环境
abstract 抽象的
...
clipboard 剪贴板
clone 克隆
CLS（Common Object Model）组件对象模型
combo box 组合框
command line 命令行
comment 注释
...
compiler 编译器
component 组件
composite index 符合索引、组合索引（for databse）
...
data dictionary 数据字典是（for database）
data file 数据文件（for database）
data integrity 数据完整性（for database）
data manipulation ianguage（DML）数据操作语言（for database）
data member 数据成员、成员变量
data source 数据源（for database）
Data source name （DSN）数据源名称（for database）
...
directory 目录
disassembler 反汇编器
DISCO（Discovery of Web Service）Web Services的查找
dispatch 调度、分派、派发
...
inner join 内连接（for database）
instance 实例
instantiated 具体化、实体化（常应用于template）
...
# L
```
left outer join 左向外连接（for database）
level 阶、层例
library 库
lifetime 生命期、寿命
